...
public class DataAnalysis  {
	...
	*/
	private String writeOccupantNumbersToFile(LocalDate start, LocalDate end, TreeMap<Room, ArrayList<Integer>> dailyTotals, TreeMap<Room, Double> averages, TreeMap<Room, Integer> totals, ArrayList<LocalDate> days) {
		String fileName = String.format("/data/dataAnalysis/%s_occupancy__occupant_numbers_%s_to_%s.csv", this.hotelName, start.toString(), end.toString());
		String filePath = System.getProperty("user.dir") + fileName;
	...
	private String writeOccupancyRoomNumbersToFile(LocalDate start, LocalDate end, TreeMap<Room, ArrayList<Integer>> dailyCounts, TreeMap<Room, Integer> totals, ArrayList<LocalDate> days, TreeMap<Room, Integer> hotelRooms) {
		String fileName = String.format("/data/dataAnalysis/%s_occupancy_room_numbers_%s_to_%s.csv", this.hotelName, start.toString(), end.toString());
		String filePath = System.getProperty("user.dir") + fileName;
	...
	/*
	 * Totals up the number of times a room was booked over the period of time i.e if 1 room was booked for 7 days that room would be mapped to 7
	 * This method is mostly just used for the dailyRoomCount method
	 */
	 ...
	 * @return
	 */
	public TreeMap<Room, Double> getAverageOccupantsPerRoomPerDay(LocalDate start, LocalDate end, ArrayList<LocalDate> days) {
		TreeMap<Room, Double> averages = new TreeMap<Room, Double>();
	...
	/**
	 * Requests the average and total occupancy information per room for all the days of the date period to be written to a file
	 * @param start the start date of the date period
	 * @param end the end date of the date period
	 * @param hotelRooms the rooms of the hotel and their counts, leave null if numberOfOccupants is true
	 * @param numberOfOccupants true if you want to retrieve number of occupants, false if you want to retrieve number of rooms booked
	 * @return the name of the file the information was stored to
	 */
	//6
	...
	/**
	 * Requests the average and total occupancy information per room for the days specified in the date range
	 * @param start the start date of the date period
	 * @param end the end date of the date period
	 * @param days the days of the date period to display total incomes for
	 * @param hotelRooms the rooms and the count in the hotel, leave null if numberOfOccupants is true
	 * @param numberOfOccupants true if you want to retrieve number of occupants, false if you want to retrieve number of rooms booked
	 * @return the name of the file the information was stored to
	 */
	public String requestOccupantInformation(LocalDate start, LocalDate end, ArrayList<LocalDate> days, TreeMap<Room, Integer> hotelRooms, boolean numberOfOccupants) {
		if (numberOfOccupants) {
			...
			String fileName = this.writeOccupantNumbersToFile(start, end, allTotals, averages, totals, days);
			return fileName.split("/")[fileName.split("/").length - 1];
		} else {
	...
	/**
	 * Like the getAverageOccupantsPerRoom() method, but instead maps the total occupants per room
	 * @param start the start date of the time period
	 * @param end the end date of the time period
	 * @param days the days to include the totals for
	 * @return a TreeMap of each room mapped to the total income generated for that room in the date period
